...
@Slf4j
@AllArgsConstructor
public class TextsEachContainsCondition implements Condition {
    ...
    private void verify(Locator locator, double timeout) {
        log.debug("Check each of '{}' contains text: {}", locator.toString(), expectedText);

        locator.all().forEach(elm ->
            assertThat(elm).containsText(expectedText, new LocatorAssertions.ContainsTextOptions()
    ...
    public String toString() {
        return "each contains text '" + expectedText + "'";
    }
}
