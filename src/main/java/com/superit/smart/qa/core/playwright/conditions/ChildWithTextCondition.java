...
@Slf4j
@AllArgsConstructor
public class ChildWithTextCondition implements Condition {
    ...
    private void verify(WebElm webElm, double timeout) {
        ...
        log.debug("Check '{}' has item with text: '{}'", elmsLocator.toString(), text);

        Locator elm = elmsLocator.getByText(text, new Locator.GetByTextOptions().setExact(false));
