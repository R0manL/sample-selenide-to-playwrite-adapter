...
public class AttributeCondition implements Condition {
    ...
    private void verify(Locator locator, double timeout) {
        log.debug("Check elm: '{}', has attribute: '{}' with value: '{}'", locator.toString(), attributeName, expectedAttributeValue);
        assertThat(locator).hasAttribute(attributeName, expectedAttributeValue, new LocatorAssertions.HasAttributeOptions().setTimeout(timeout));
    }

    @Override
    public String toString() {
        return "attribute '" + attributeName + "' has value " + expectedAttributeValue;
    }
}
