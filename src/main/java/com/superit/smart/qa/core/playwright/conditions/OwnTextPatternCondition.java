...
@Slf4j
@AllArgsConstructor
public class OwnTextPatternCondition implements Condition {
    ...
    private void verify(Locator locator, double timeout) {
        log.debug("Check '{}' contains own text (ignore case) match pattern: '{}'", locator.toString(), pattern);
        assertThat(locator).containsText(pattern, new LocatorAssertions.ContainsTextOptions()
    ...
    public String toString() {
        return "contains own text (ignore case) match pattern '" + pattern + "'";
    }
}
