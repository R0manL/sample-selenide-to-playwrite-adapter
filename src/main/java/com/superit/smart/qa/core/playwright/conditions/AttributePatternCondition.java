...
@Slf4j
@AllArgsConstructor
public class AttributePatternCondition implements Condition {
    ...
    private void verify(Locator locator, double timeout) {
        log.debug("Check elm: '{}', has attribute: '{}' with value match pattern: '{}'", locator.toString(), attributeName, pattern);
        assertThat(locator).hasAttribute(attributeName, pattern, new LocatorAssertions.HasAttributeOptions().setTimeout(timeout));
    }

    @Override
    public String toString() {
        return "attribute '" + attributeName + "' has value match pattern " + pattern;
    }
}
