...
@Slf4j
@AllArgsConstructor
public class ValuePatternCondition implements Condition {
    ...
    private void verify(Locator locator, double timeout) {
        log.debug("Check elm: '{}', has value that match: '{}'", locator.toString(), pattern);
        assertThat(locator).hasValue(pattern, new LocatorAssertions.HasValueOptions().setTimeout(timeout));
    }

    @Override
    public String toString() {
        return "has value match'" + pattern;
    }
}
