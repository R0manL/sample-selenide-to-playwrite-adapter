...
@Slf4j
@AllArgsConstructor
public class ValueEndWithCondition implements Condition {
    ...
    private void verify(Locator locator, double timeout) {
        log.debug("Check elm: '{}', contains (partial) value: '{}'", locator.toString(), expectedValue);
        assertThat(locator).hasValue(Pattern.compile(".*" + expectedValue + "$"), new LocatorAssertions.HasValueOptions().setTimeout(timeout));
    }
