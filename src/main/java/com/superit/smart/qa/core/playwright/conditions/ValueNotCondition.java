...
@Slf4j
@AllArgsConstructor
public class ValueNotCondition implements Condition {
    ...
    private void verify(Locator locator, double timeout) {
        log.debug("Check elm: '{}' has not value: '{}'", locator.toString(), expectedValue);
        assertThat(locator).not().hasValue(expectedValue, new LocatorAssertions.HasValueOptions().setTimeout(timeout));
    }

    @Override
    public String toString() {
        return "has not value '" + expectedValue;
    }
}
