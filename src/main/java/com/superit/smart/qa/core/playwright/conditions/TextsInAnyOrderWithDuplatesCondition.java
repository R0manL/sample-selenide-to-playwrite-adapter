...
@Slf4j
@AllArgsConstructor
public class TextsInAnyOrderWithDuplatesCondition implements Condition {
    ...
    private void verify(WebElm webElm, double timeout) {
        ...
        log.debug("Check '{}' has inner texts (ignore case, any order):'{}'", elmsLocator.toString(), texts);
        assertThat(elmsLocator).hasCount(texts.size());

        texts.forEach(text -> {
            Locator elm = elmsLocator.getByText(text);

            // We have more than one element with same text.
            if (elm.all().size() > 1) {
                elm = elm.nth(0);
            }
    ...
    public String toString() {
        return "has texts (ignore case, any order)'" + texts + "'";
    }
}
